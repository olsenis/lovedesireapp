...
- **Sep 14 2026 (USER_VOICE A1):** accept moved to the `acceptPairing` callable; the client can no longer fill a slot (rules). A NEW partner after a disconnect gets a fresh couple doc (old one archived, remaining partner keeps read access), the SAME partner refills the old doc. `partnerLeftUid` is stamped by every disconnect path. pairing.tsx detects acceptance from its own profile and lost its Retry sub-view. profile.tsx no longer disconnects before joining or writes `coupleId` on request.
