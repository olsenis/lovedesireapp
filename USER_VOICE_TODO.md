...
| ✅ C3 (Sep 19, pulled forward) | Past answers archive: a browsable list of Daily and Sunday answers by week | 95 "look back / old answers" mentions; Agapé's scrapbook is what its loyal users cite; Evergreen "no ability to see how our relationship has grown" | new `app/answers-archive.tsx` reading `dailyQuestions` and `stateUnion` by date range; entry from Our Story | M | ⬜ |
| ✅ C4 (Sep 16) | Custom mood label + the neurodivergent set (five new free moods, 💬 own words on both pickers, `moodLabel`) | Cozy Couples' most repeated wish (dozens): "more moods", "custom moods", "'off', 'in my head', 'overthinking', 'missing you', 'check in with me'" | `services/moodService.ts` MOOD_LABELS + a `customLabel` on the entry; Home mood picker | S | ⬜ |
...
| ✅ C12 (Sep 16) | "Draw one of our matches": a seeded pick from Fantasy Wishes matches, same on both phones the same day | r/sexover30 wish; turns the match list into a Tonight suggestion | `app/fantasy-wishes.tsx` Matches tab button | S | ⬜ |
...
| ⬜ C14 add-on (Sep 21) | Love Language result screen: more "Try this" tips per pair so the compatibility card can rotate monthly (the quiz is one time by design; the weekly Monday actions, the daily insight card and "Need an idea?" are what recur). Pure content work, 15 pairs. Also possible: a yearly Home card "A year since your Love Language quiz. Still true?" like the Lovers anniversary card | Óli asked about retention on the result screen, Sep 21 | `constants/content.ts` LOVE_LANGUAGE_COMPATIBILITY, `app/quiz.tsx` | M | ⬜ |
| ✅ C2b (Sep 16) | "Ask {partner} something" (the link reads "Write your own question ›" since Sep 20): one couple-written question a day in Daily, appended to the deck, same mutual reveal (the write-your-own wish, 48 mentions, 44 positive; we have it in WYR, Activity Cards and Truth or Dare, not in Daily) | USER_VOICE §9b wishes; "if they add that it'll be priceless" (Candle review) | `dailyQuestionsService` + `app/daily.tsx` add sheet; rules: items append by a member | M | ⬜ |
| C14 | Add questions and picks, free tier first: Playful questions 87 → 150+, Sweet picks 60 → 100, then WYR Playful / Romantic (added Sep 15 by Óli); also Sunday Check-in sets 25 → 40 before a launch-day couple reaches week 25 (about month 5), written against the three-test bar in `memory/sunday_checkin_prompt.md`, each with a tier; Fantasy Wishes presets now reach existing couples automatically (storage change Sep 19), so new cards can simply be added to the pool | A5 gives 56 days without repeats only when the pool allows it; Playful keeps ~25 days, Sweet picks ~11. Repetition was the decay complaint at ten apps (§2.4) | `constants/content.ts` with `memory/question_writer_prompt.md`; POST_LAUNCH "Grow content pools" has the per-pool targets | M content | ⬜ |
| C15 | Scratch Cards: a partner sends a card to scratch open (voucher, own text, position), kept in a wallet until redeemed, no expiry (idea from Óli, Sep 17) | A gift from one person to the other, works when the partner is not in the app, not chat; one competitor has plain "love coupons" | Design in [POST_LAUNCH.md](POST_LAUNCH.md) "Scratch Cards"; build after round 3 and the first EAS build; positions art only after an App Review check | M (6 h) + pool | ⬜ |
