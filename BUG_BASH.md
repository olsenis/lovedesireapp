...
| 2 | **Ratings prompt** with `expo-store-review` after a happy moment (Fantasy Wishes match, Sunday reveal, Memory Lane score, 30-day program done); gated on 7 days since install, 3 rituals completed, 90 days between asks. MARKETING §2.9. | Claude | 1 h | ✅ `services/reviewPromptService.ts`, no-op in Expo Go, verify on the first EAS build |
| 3 | **Send invite** share sheet in pairing + `/join` page on the site with the store badge and the code. MARKETING §5. | Claude | 1.5 h | ✅ pairing "Send invite" button, `web/src/pages/join.astro`; device: tap → share sheet, message carries code + link, link opens /join with the code |
| 4 | **Expiring mutual-only mood**: a mood that clears itself after N hours and pushes only when both match. POST_LAUNCH C1 #2, pulled forward. | Claude | 3 h | ✅ "Tonight? 🔥" pill on the couple card, `services/tonightService.ts`, rules deployed; two-phone test in TEST_CHECKLIST §2 |
| 5 | **Joint consent screen before a Spicy session** (Truth or Dare Spicy, Fantasy Wishes): one shared "we both want this tonight" tap on top of the per-user 18+ attestation. POST_LAUNCH C1 #8, pulled forward. | Claude | 2 h | ✅ `hooks/useSpicyConsent.tsx`: joint copy in Together Right Here, per-phone copy in Wherever You Are and Fantasy Wishes, once per day; tests in TEST_CHECKLIST §5 and §6 |
| 6 | **Say the differentiators**: "No streaks" on the paywall and screenshot 2; "not for everyone" block on `/about`; "no card needed" next to the trial; more free mood options. | Claude | 1 h | ✅ paywall note, /about block, trial line ("cancel during the trial and you pay nothing", not "no card": App Store trials need an Apple ID payment method), moods Cuddly / Playful / Grateful |
...
**Two-phone round 3, Sep 19 to 20 2026 (Oli + Ola), found and fixed on the spot:** Fantasy Wishes cards print their category and 12 location cards moved from roleplay to explicit (`39be2f3`); Mood History stat cards collapsed on RN 0.86, now a fixed share of the row (`37da862`); Reset: the person who asked never learned the answer, now the request becomes an answer the asker reads on the Reset row and a Home card, for yes and for not now (`cedae1f`), and reading it closes it, the first "OK" button made no sense on a phone (`168fb65`); Home couple card pushed the partner's avatar off a 388dp screen, plus "1 yrs" (`5758558`, with a small-screen pass added to TEST_LAUNCH); Terms §3 and §9 contradicted the erasure rule (`7885b2b`); peer check on deletion of shared data in LAUNCH_STATUS §4b (`e309f8e`): keep the rule and the seven days; Delete account sheet and the breakup FAQ still described the old behaviour (this commit). PASSED on two phones: FW category switch syncs, "+ Add to Together List" makes one todo, Reset "Clear mine", "Clear for both" yes and not now. NEXT: Intimacy Log row in Reset.
...
|---|---|---|---|---|
| P0 | **New partner never inherits the old partner's data**: `acceptPairing` fills the empty slot of the old couple doc after a disconnect, so the next partner reads the previous partner's Intimacy Log, Fantasy Wishes votes, Sunday answers, notes and Moments. Fresh couple unless the joiner is `partnerLeftUid`. Both cases into TEST_LAUNCH. | Lasting review (data leak to a new partner); USER_VOICE theme 1 | 4 h | ✅ Sep 14: `acceptPairing` callable (fresh doc for a new partner, same doc for the same partner), rules make slots client-immutable, `partnerLeftUid` on every disconnect path; three tests in TEST_LAUNCH §1 + TEST_CHECKLIST §1, two-phone run pending |
| P1 | **Lapsed subscription keeps read access** to own Intimacy Log, FW matches, The Lovers result; only new writes gated. Rule into the CLAUDE.md paid-gate pattern. | Cozy Couples 2026 "cash grab" wall; theme 3 | 4 h | ✅ Sep 14: `hooks/usePaidAccess` (write gate + read view), `PremiumEndedBanner`, read views on Intimacy Log / FW matches / Lovers result / Activity Cards board, cards route to the screen, write-nudges gated; TEST_CHECKLIST §13 |
| P2 | **Billing honesty**: trial end date + renewal price on the paywall, "Manage subscription" in Profile, price in the store description. | 448 negative reviews in 13 apps; theme 2 | 2 h | ✅ Sep 14: `app/upgrade.tsx` plan rows + "Free until {date}, then {price}" line, Profile "Manage subscription" (store link), MARKETING §2.10 Premium paragraph, site trial line; TEST_CHECKLIST §11 |
| P3 | **Discreet pushes**: reword Tonight and FW match bodies; "Discreet notifications" toggle. | theme 11 | 1 h | ✅ Sep 15: recipient-side `discreetNotifications` (default on) picks a discreet variant on the eight sensitive pushes; Profile switch; TEST_LAUNCH §7 lists both wordings |
| P4 | **Daily no-repeat** over 8 weeks, seeded, like Memory Lane's `recentQuestionIds`. | theme 4; 87-question free pool | 3 h | ✅ Sep 15: `excludeRecent` (seed.ts) on Daily questions and picks, 56 days, shrinks per category so a small pool never empties; both phones agree via the same history read; dev flag for a 3-day window; TEST_CHECKLIST §5 |
| P5 | **Fantasy Wishes opt-out**: "Skip scenarios that involve other people" + category chips. | Spicer's top 2026 complaint | 3 h | ✅ Sep 15: `category` on every preset, `couples.fwCategories` per couple, deck gentle-first, ☰ sheet; no "others" switch needed (no third-party scenarios in the pool); TEST_CHECKLIST §6 |
...
Shipped from §C ahead of schedule: **C2 reactions + replies** (Sep 15, `ReactionRow` on Daily, Fantasy Wishes matches, Sunday Check-in; rules deployed); **C2b Ask {partner} something in Daily** (Sep 16, 9a21b4c, `custom` map on the day doc, rules deployed); **C6 Home while unpaired** (Sep 16, f25a7bd, "While you wait" card, Tonight's Picks hidden until paired); **C4 five more moods + own words** (Sep 16, 6dd9c7b); **C12 draw one of our matches** (Sep 16, a74f334). Test items for all four are in TEST_CHECKLIST under their feature headings. **Evu-listinn round 3** (Sep 17, 21 items: three sittings by Premium state, the two dated items carried over from round 2, the A1 disconnect test last) is live at /qa/evu-listinn.html; a sixth group "Dev build" (items 22 to 27: push arrives, discreet on / off, the Daily ask push, Face ID, the ratings prompt) waits for the first iPhone build (86427ac). **Feature hints audit** (Sep 17): 13 hints rewritten against the current screens (The Lovers type names, Tonight's Date count, Challenge edits and vetoes, plus everything shipped since Aug), 8 added (Sunday Check-in, Memory Lane, Moments, Intimacy Log, Tease, Special Days, Our Story, Mood History); sweep item in TEST_CHECKLIST. **Sep 19, from Óli's fresh-account run:** the invite code wrapped to two lines (4948f1f, one line with auto-shrink, also on Home), and the 18+ gate flashed for a second on a new account (a race between the consent read in the root layout and Register's write; three guards, test items added). Same run, Sunday Check-in: the pulse button was half width and the buttons sat under the Android keyboard (both fixed, the screen now uses Daily's keyboard-aware scroll); the question pool got depth tiers (light sets for a couple's first three check-ins, heavy ones from the ninth) and a full content pass, all 125 questions rewritten against a three-test bar after "When has my flexibility felt like love to you?" stopped Óli cold. **C3 pulled forward (Sep 19):** the text answers to Daily questions were kept but unreadable after midnight; Our Story now has a fifth archive, "Daily answers", with a link from Daily's end-of-day state. **Call it retired (Sep 19):** Óli read the hidden predictions as a wish list the partner then has to fail; replaced by "A little something for {partner}", a private plan about your own behaviour where only what you ticked as done is ever shown (`services/sundayPlanService.ts`, plan in `users/{uid}/private`). **A1 PASSED on two phones (Sep 19):** a new partner sees nothing of the former partner's data, on either side. Still to run: the same partner coming back gets the history back. **A1 gap found in the same test:** a new partner's fresh couple doc started without Premium, so the payer lost it by re-pairing; `acceptPairing` now carries Premium to its owner (deployed), and the RevenueCat webhook requirements are written into LAUNCH_STATUS §4. **Fantasy Wishes storage changed (Sep 19), found by Óli asking why a new couple sees "Loading":** the 394 presets are no longer copied into each couple; the deck comes from the app and the couple has one `fwState/main` doc (votes, matches, hearts), like Daily. No loading, one read per open instead of 394, new presets reach everyone, create-time vote forging closed in the rules (deployed). **Test couples start Fantasy Wishes fresh: old test votes and matches are not carried over**, and the old copied docs are deleted in the background on first open. **Deck order (Sep 19):** every preset rated 1 to 3 and the deck now starts gentle, mixes the four categories and builds (it used to be 50 Sensual, then 67 Roleplay, then 209 Explicit in authoring order); the TypeScript order is checked against a simulator card for card. **Reset screen (Sep 19):** a couple can start over in one part of the app from Profile → Reset (four rows one person may clear, six that need the partner to agree); all deleting is in the new `resetCoupleData` callable (deployed, invoker IAM applied). Fantasy Wishes' ↺ is gone from its header. **Rebuilt the same evening after Óli asked whether one partner blocking a deletion is a privacy violation (it was, and the reverse too):** Reset now follows "yours alone and at once, your partner's with agreement, about-both by either of you after seven days"; account deletion erases the leaver's own contributions (it used to keep everything, against Privacy §6); the Intimacy Log switch offers the erasure; policy (app + site), FAQ, support page and DPIA rewritten to match. All server-side, deployed. Pool 394 → 392: the one card that mentioned other people is gone (so "every scenario is for the two of you" is now simply true) and two humiliation cards became one. Full detail (where, steps, tests) per item: [USER_VOICE_TODO.md](USER_VOICE_TODO.md) §A (P0 = A1 … P7 = A8, plus A9 help line and A10 tests). Tick there and here together.
