...
**Intimacy:** The Lovers (entire feature), Sensate Focus (entire
...
- [ ] **Small-screen and large-text pass** (Sep 20 2026, after two layout bugs that only showed on the narrower of two test phones: Mood History stat cards, Home couple card). On ONE Android phone set Settings → Display → Display size one step up and Font size one step up (this makes a 360dp screen with larger text, close to an iPhone SE), then walk: Home (couple card, nudge cards), Daily, Fantasy Wishes (card + Matches), Sunday Check-in (all steps incl. buttons), Mood History both tabs, Our Story tiles, Reset, Profile, Upgrade, every bottom sheet. **Fail:** anything clipped at the right edge, text one character per line, a button label missing, a primary button pushed off the screen. The rule for new rows: side columns get a fixed `width`, the middle gets `flex: 1, minWidth: 0`, and two equal cards get a percentage width; never rely on `minWidth` plus text that cannot shrink.
...
- [ ] **Blueprint quiz completion + couple compatibility view** 📱 💰
  1. Both (premium): Open The Lovers → answer all 15 questions → see result
