...
couples/{coupleId}/stateUnion/{weekId}/entries/{uid} StateUnionEntry — answers{qi:text}, pulseScores?, predictions?[] (Sep 2026: up to 3 calls about the partner for the coming week), reactionsOnPartner?{qi:true} + repliesOnPartner?{qi:text} (C2, my heart / one line on the partner's answer, same owner-write / read-after-both gate), verdictsOnPartner?{idx:bool} (MY verdicts on the PARTNER's predictions from this SAME week W, written from a later week once W is both-complete; the screen looks back up to 3 weeks so a skipped week orphans nothing, Review #11 B6), updatedAt (rules: only readable by owner OR after both completed; owner-write has no week restriction, which is what lets a later week grade an earlier one)
