...
app/                         Full-screen sub-screens
  ...
  calendar.tsx               Special Days ledger — chronological list of anniversaries, birthdays, first times, grouped by Coming up / Next 3 months / Later this year. Rewritten from month-grid Aug 2026 to a pure ledger. Includes auto-inline Valentine's, partner birthday, couple anniversary from startDate. Secret dates render as "A surprise from {partnerName}" until the day arrives. **🔔 per row (Sep 24 2026):** two local notifications on THIS phone for the next occurrence, a week before and on the day at 09:00 (`services/specialDayReminderService.ts`, preference in AsyncStorage, per phone, nothing in Firestore; a partner's secret date keeps its placeholder in the body). DATE triggers fire once and the screen rebooks the next occurrence whenever its ledger is computed, so a phone that never opens Special Days again stops after the next occurrence.
  ...
  reminders.tsx              Reminders (called "Flirt Reminders" until Sep 21 2026; most of them are sweet, not flirty) — private reminders to do something sweet for the partner, local scheduled notifications, **Weekly** (time + weekdays) or **Once** (time + a date, dropped from the list after it fires; Sep 24 2026). Save is disabled until a day or a date is chosen. Opened from the Us tab (last section, after "Speak {partner}'s language"); was under Profile until Sep 21 2026.
  ...
  love-language-nudge.tsx    Speak partner's love language — weekly Sunday nudge (local scheduled notification, Sunday 09:00), 3 concrete actions from partner's language pool (20 per language × 5 languages = 100). Deterministic pick (week + coupleId, nothing stored) so both partners see the same trio, dealt like a deck since Sep 21 2026: one seeded shuffle per cycle, three a week, no repeat inside a cycle and nothing back sooner than 4 weeks (20 actions = 6-week cycles; before, each week was an independent shuffle and about 4 weeks in 10 repeated something from the week before). Our Story recomputes past weeks from the same function, so AFTER LAUNCH never change `pickWeeklyActions` or reorder a pool.
...
users/{uid}/private/flirtReminders   { items: FlirtReminder[] } — message, time, days[], date? (YYYY-MM-DD for a once-reminder, `days` then empty), active, createdAt, local id (Sep 21 2026: PRIVATE, a reminder to do something for the partner only works if the partner never sees it; max 12)
...
| `clock.ts` | `formatClock(date, timeZone?)`, `formatClockHHmm('18:00')` — a time of day as THIS phone shows it (12 or 24 hour follows the phone's setting). Use for every clock the user reads (Reminders, Love Notes, the LDR clock, Tonight, the date picker); never force `en-GB` / `hour12: false` for display (that is only right when the hour is used as a number). Sep 24 2026. |
...
- `LOVE_LANGUAGE_TYPE_CONFIG` (per language: `description` speaks to the person whose result it is, `aboutPartner` says the same ABOUT the partner and is what the partner card and the Monday screen show) + `LOVE_LANGUAGE_COMPATIBILITY` (15 pair guides) + `fillLoveLanguageRoles`. Rewritten Sep 21 2026 in plain English. **Who is who:** in `summary` / `challenge` a role token names a person by language (`{gifts}`, `{Time}`…) and is filled with "you" for the viewer's own language and the partner's name for the other; write a token only before a modal or past form ("{gifts} may spend", never "{gifts} spends" or "{gifts}'s"). A tip is a string (both) or `{ for, text }` (advice to the person with that language); each viewer sees the shared tips and their own. See `memory/love_language_compatibility_prompt.md`.
