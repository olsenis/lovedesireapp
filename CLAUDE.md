...
**Home Tonight's Picks:** 3 launcher tiles — Daily (💫), Truth or Dare (🎯), Fantasy Wishes (✨). See-all-games row routes to Discover tab. Dares tile was added Aug 2026 (H5) to close a discoverability gap, then removed Aug 2026 (H14) when async dares got consolidated into the Truth or Dare picker as "Send a Dare" mode — the T-or-D tile now owns the entry. Home nudges for in-flight dares route to `/truth-dare` where the AsyncDaresPanel below the mode cards surfaces both incoming and sent dares (see H18 note in the Truth or Dare multiplayer section above).
