...
- `DARES` + `DARE_LEVEL_CONFIG` — 274 dares across Sweet (70) / Flirty (81) / Spicy (123). Clear level separation: Sweet=cute/romantic, Flirty=sensual kissing/touch, Spicy=explicitly sexual/X-rated. Every dare has an optional `remote?: boolean` — `true` = LDR-safe (voice memo, video call, camera, text, coordinated ritual, solo-and-report-back), unset/false = requires physical proximity. `truth-dare.tsx` derives `daresPool = isLDR ? DARES.filter(d => d.remote) : DARES` at the top of the screen and every DARES read flows through it, so long-distance couples never draw a physical-together dare. Remote-safe counts: Sweet 38, Flirty 38, Spicy 35 — comparable per-level variety across the isLDR toggle. Truths need no LDR filter since they're all verbal/typed/audio by nature. See `memory/explicit_content_prompt.md` and `memory/question_writer_prompt.md`.
...
**Home Tonight's Picks:** 3 launcher tiles — Daily (💫), Truth or Dare (🎯), Fantasy Wishes (✨). See-all-games row routes to Discover tab. Dares tile was added Aug 2026 (H5) to close a discoverability gap, then removed Aug 2026 (H14) when async dares got consolidated into the Truth or Dare picker as "Send a Dare" mode — the T-or-D tile now owns the entry. Home nudges for in-flight dares still deep-link to /dares directly, so an active-dare signal never gets buried.
