...
couples/{coupleId}/resetRequests/{key} ResetRequest — uid (who asked), at, autoAt? (joint targets only: when it clears without the partner; swept hourly by `runDueResets`). Written ONLY by the `resetCoupleData` callable; members can read, never write (rules). One per reset key, lapses after 7 days.
