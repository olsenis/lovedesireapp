...
8. **Joint consent screen before a Spicy session** (from koopla): one shared "we both want this tonight" tap before Truth or Dare Spicy or Fantasy Wishes, on top of the per-user 18+ attestation. ~2 h, and a good App Review story. Shipped Sep 13 2026 (`useSpicyConsent`).

**From USER_VOICE.md (review mining, Sep 13 2026):** the post-launch order (widgets, ~~react + reply on a revealed answer~~ (shipped Sep 15, pulled forward), ~~ask your partner something in Daily~~ (Sep 16), ~~past-answers archive~~ (Sep 19, pulled forward), ~~custom mood label + five more moods~~ (Sep 16), lifetime SKU, ~~solo first week~~ (Sep 16), data export, intensity control, "who initiates" in Sunday, life-stage tags and body configuration, Play decision, ~~random match draw~~ (Sep 16), review replies) lives in [USER_VOICE_TODO.md](USER_VOICE_TODO.md) §C with evidence and effort per row; the pre-launch items are §A there and P0 to P7 in BUG_BASH.

### Scratch Cards (idea from Óli, Sep 17 2026; USER_VOICE_TODO C15)

A partner sends a card the other scratches open with a finger. It hides a voucher (free date, back rub, breakfast in bed, an explicit one), text of the sender's own, or a position. Why it fits: it is a gift from one person to the other and not a system prompt, it has a small moment of suspense, it works when the partner is not in the app at the same time, it is not chat, and the wallet gives the explicit side something that lasts longer than one evening. One kitchen-sink competitor has "love coupons" (COMPETITORS.md), with no scratch moment and no explicit tier.

- **Flow:** Us tab (Nurture) card "Scratch Cards". Sender picks a kind, picks or writes the content, sends. Receiver gets a push (discreet: "{name} sent you a card", never the content), scratches the foil, and the card moves to "Yours to redeem". Redeem → confirm modal → the sender gets "{name} redeemed your card" and the card moves to Redeemed. A card can also be saved to the Together List.
- **Kinds and tiers:** sweet vouchers and own text free; explicit vouchers Premium; positions Premium and behind the Spicy consent card (`useSpicyConsent`). Free couples one card a week, Premium unlimited (decide at build time; new feature, so D2 is not touched).
- **Data:** `couples/{id}/scratchCards/{id}`: `fromUid, toUid, kind ('voucher'|'text'|'position'), text, presetId?, createdAt, scratchedAt?, redeemedAt?`. Rules: create only with `fromUid == me`; `scratchedAt` / `redeemedAt` writable only by `toUid`; text immutable after create and bounded like `bingoCustom`. Report entry point (`useReport`) on own-text cards.
- **Scratch effect:** a `react-native-svg` Mask over the foil, fed by the finger path from `react-native-gesture-handler`; reveal fully at about 55% scratched, haptic on reveal. All three libraries are installed and work in Expo Go.
- **Content:** voucher pools of about 30 each, sweet with `memory/question_writer_prompt.md`, explicit with `memory/explicit_content_prompt.md`. Positions: a name plus one sentence, about 40, text first.
- **Positions art:** possible later as abstract, faceless line figures in SVG (no genitals, no faces, one stroke colour), never in screenshots or previews, only behind Premium and the consent card. App Review risk is real and NOT verified: stylised position guides have lived on the App Store at 17+ and some have been pulled. Check guideline 1.1.4 as it reads then and two or three live examples before commissioning any art.
- **Voice rules:** no expiry, no count of unredeemed cards, no reminders about them. The words are "card" and "redeem", never "owe".
