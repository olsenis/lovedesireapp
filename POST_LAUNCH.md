...
**From USER_VOICE.md (review mining, Sep 13 2026):** the post-launch order (widgets, ~~react + reply on a revealed answer~~ (shipped Sep 15, pulled forward), past-answers archive, custom mood label, lifetime SKU, solo first week, data export, intensity control, "who initiates" in Sunday, life-stage tags and body configuration, Play decision, random match draw, review replies) lives in [USER_VOICE_TODO.md](USER_VOICE_TODO.md) §C with evidence and effort per row; the pre-launch items are §A there and P0 to P7 in BUG_BASH.
