...
- [ ] **The partner never sees them** 📱 ⚠️
  1. Phone A: Reminders → tap "Tell Eva one thing you love ❤️" → save
...
- [ ] **Clock follows the phone** : set the phone to 12-hour time (Android: Settings → General management → Date and time → Use 24-hour format off; iOS: Settings → General → Date & Time) → Reminders rows, the time field, the Android clock dial, a timed Love Note ("Opens Thursday 24 Sep at 6:00 PM"), the LDR clock on the couple card and "clears at" on Tonight all read 12-hour with AM/PM; set 24-hour → all read "18:00". (Sep 24 2026: everything was forced to 24-hour.)
...
- [ ] **Empty**: a new couple sees "Nothing here yet…" and no spinner that never ends; an unpaired account the same.
...
- [ ] **Clear for both needs the partner** 📱
  ...
  - **Expected:** A's row showed "Waiting for Eva to agree. Your own part you can clear right now." with Cancel. After B agrees everything is gone for both, Storage prefix empty, request and Home card gone. **The asker reads the answer:** after "Agree and clear" A's row says "Eva agreed. It is cleared for both of you…" and A's Home has a card "Eva agreed to clear Moments"; after "Not now" the row says "Eva said not now, so nothing was cleared. Your own part is still yours to clear." and nothing is deleted. There is no button: when A leaves the Reset screen the answer is gone from the row and from Home, and "Clear for both" was available the whole time. B never sees a card about B's own answer. Dev build: the push to A reads "Eva answered you" for both answers.
...
- [ ] **Only what was done is shown** 📱
  ...
  - **Expected:** B's reveal shows "🎁 A little something from Oli" with the item, also under Past check-ins and in Our Story → Sunday check-ins. Repeat with nothing ticked: no card, no text, no count anywhere on B; the plan is not asked about again on A.

- [ ] **Former partner's plan is not offered** : a plan made while paired with Eva does not appear after pairing with Ola.

- [ ] **"Need an idea?" fills the name** : tap it in the plan step → the field reads "...with Eva", never "{partner}"; the plan card, the tick card, the Monday screen and the partner's "🎁 A little something from …" never show a "{partner}" token (a done item written before Sep 21 2026 reads "...with you" on the partner's side). With nothing ticked the tick card's button reads "Not this time", with a tick "Save". The reveal says "See you next Sunday". The pulse comparison uses the partner's first name and its labels stay on one line on a narrow phone.
- [ ] **The plan is shown back to me** : after finishing the check-in with a plan, the finished screen shows "Your little something for Eva · only you see this" with my items, whether or not Eva has finished; on Monday (a new ISO week) opening the Sunday Check-in still shows the card on the first step; Eva's phone never shows it; once I finish the next check-in the card gives way to "Last time you planned" with ticks, and after Save it is gone. Past check-ins never lists a plan, only "🎁 A little something from …" items.
